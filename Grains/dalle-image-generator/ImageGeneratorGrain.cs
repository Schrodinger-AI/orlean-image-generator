...
public class ImageGeneratorGrain : Grain, IImageGeneratorGrain, IDisposable
{
    ...
    private async Task<DalleResponse> RunDalleAsync(string prompt)
    {
        ...
        try
        {
            ...
            _logger.LogInformation(
                "t-01 ImageGeneratorGrain - generatorId: {RequestId} , Dalle API responseCode: {responseStatusCode} - responseReason: {responseReason} - response: {response}", _imageGenerationState.State.RequestId, response.StatusCode, response.ReasonPhrase, response);
